...
def process_file(path, key_bytes)
  ...
  # Write the encrypted data and checksum to .npc and .sum
  File.write("#{dir}/#{base}.npc", xored)
  File.write("#{dir}/#{base}.sum", checksum)

  puts "Packaged #{base} -> .npc, .sum"
end

def main()
  puts '%%% NPCPacker for NeonDreams %%%'
  puts 'Remember that the key is in the source!'
  hex_input = '0x08DCA670'
  ...
  Dir.glob('./npc/*.json').each do |json_file|
